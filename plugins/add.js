...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
  ...
  if (response[users] == 408) throw `The number has been dialed recently\nCan only enter through ${usedPrefix}link`
  ...
  for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
    ...
    await conn.sendGroupV4Invite(m.chat, jid, invite_code, invite_code_exp, true, 'Invitation to join my WhatsApp group', jpegThumbnail ? {
      jpegThumbnail
    } : {})
  }
}
